...
//
//void jni_audio_prepare(JNIEnv *pInterface, jobject pVoid, struct Player *pPlayer);
//
///**
// * 初始化解码器上下文
// */
//void init_codec_context(struct Player *play, int index) {
//    AVFormatContext *format_ctx = play->input_format_ctx;
//    //获取解码器
...
//        return;
//    }
//    AVCodecContext *codecContext = format_ctx->streams[index]->codec;
...
//    }
//
//    play->input_codec_ctx[index] = codecContext;
//
//}
//
//
...
//
//    ANativeWindow_Buffer output_buffer;
//    AVCodecContext *code_ctx = player->input_codec_ctx[player->video_stream_index];
//    int got_frame;
//    avcodec_decode_video2(code_ctx, yuv_frame, &got_frame, packet);
//
//
//    if (got_frame) {
//        ANativeWindow_setBuffersGeometry(player->nativeWindow, code_ctx->width, code_ctx->height,
//                                         WINDOW_FORMAT_RGBA_8888);
//        ANativeWindow_lock(player->nativeWindow, &output_buffer, NULL);
//
//        avpicture_fill((AVPicture *) rgb_frame, output_buffer.bits, AV_PIX_FMT_RGBA,
//                       code_ctx->width, code_ctx->height);
//
...
//    int got_frame;
//    avcodec_decode_audio4(codec_ctx, frame, got_frame, frame);
//    //16bit 44100 PCM 数据（重采样缓冲区）
//    uint8_t *out_buffer = (uint8_t *) av_malloc(MAX_AUDIO_FRME_SIZE);
//
//    if (got_frame > 0) {
//        swr_convert(pPlayer->swr_ctx, &out_buffer, MAX_AUDIO_FRME_SIZE, frame->data,
//                    frame->nb_samples);
//        //获取sample 的size
//        int out_buffer_size = av_samples_get_buffer_size(NULL, pPlayer->out_channel_nb,
//                                                         frame->nb_samples, pPlayer->out_sample_fmt,
//                                                         1);
//        //关联当前线程的JNIEnv
...
//
//
//JNIEXPORT void decode_audio_prepare(struct Player *pPlayer) {
//    AVCodecContext *codec_ctx = pPlayer->input_codec_ctx[pPlayer->audio_stream_index];
//    //输入采样率
...
//    pPlayer->swr_ctx = swr_ctx;
//}
//
//void JNICALL Java_comasd_example_demopthread_MainActivity_ffmpeg
//        (JNIEnv *env, jobject jobj, jstring input, jobject surface) {
...
//
//    //初始化封装格式上下文
//    init_input_format_ctx(player, input_char);
//
//    int video_index = player->video_stream_index;
//    int audio_index = player->audio_stream_index;
//
//    init_codec_context(player, video_index);
//    init_codec_context(player, audio_index);
//    decode_audio_prepare(player);
//    decode_video_prepare(env, player, surface);
//    jni_audio_prepare(env,jobj,player);
//    pthread_create(&(player->decode_threads[video_index]), NULL, decode_data, (void *) player);
//}
//
...
JNIEXPORT void JNICALL Java_comasd_example_demopthread_MainActivity_ffmpeg
